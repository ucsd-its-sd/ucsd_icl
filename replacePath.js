...
fs.readFile(txtPath, { encoding: 'utf8' }).then((txtContents) => {
  const dataStatus = term.padEnd(9, " ") + "Version\n.A00Ari,Stassinopoulos\n0035APM  12345LE1313 08001930A00";
  fs.writeFile(txtPath, txtContents.split(/\n|\r/g).map(
    (line, idx) => {
      if (idx == 1) { return dataStatus + "\n" + line; }
      return line;
    }).join('\n')
  );
});
...
fs.readFile(htmlPath, { encoding: 'utf8' }).then((htmlContents) => {
  ...
  fs.writeFile(htmlPath, htmlContents.split(/\n|\r/g).map((line) => {
    if (nextLineReplace) {
      console.log("replace line: `" + line + "` with `    window.icl.CURRENT_PATH = \"" + txtHTMLPath + "\";`");
